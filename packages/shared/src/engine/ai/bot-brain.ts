...
export function standings(
  state: GameState,
): { playerId: PlayerId; territories: number; armies: number }[] {
...
export function troopToBuild(
  ...
  const options: { territoryId: TerritoryId; unit: UnitKind; score: number }[] = [];
...
export function reinforcementPlan(
  ...
  profile: BotProfile | undefined = 'oportunista',
  bias?: StrategyBias,
): { territoryId: TerritoryId; armies: number; reason: string }[] {
  if (reserve <= 0) return [];
  const traits = traitsOf(profile);
  ...
  const plan: { territoryId: TerritoryId; armies: number; reason: string }[] = [];
